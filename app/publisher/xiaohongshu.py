...
class XhsPublisher:
    ...
    async def _upload_via_browser(self, image_path: str, title: str, description: str) -> None:
        async with async_playwright() as p:
            ...
            # 填写标题
            title_input = page.locator('input[placeholder*="标题"], div[contenteditable="true"]').first
            await title_input.fill(title)

            # 填写描述
            desc_input = page.locator('div[contenteditable="true"]').nth(1)
            if await desc_input.count() > 0:
                await desc_input.fill(description)

            # 点击保存草稿（不发布）
            draft_btn = page.locator('button:has-text("存草稿"), button:has-text("保存草稿")')
